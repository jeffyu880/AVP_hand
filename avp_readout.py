...
class AVP:
    ...
    def get_frame(self):
        r = self.avp.latest # dict_keys(['left_wrist', 'right_wrist', 'left_fingers', 'right_fingers', 'head', 'left_pinch_distance', 'right_pinch_distance', 'right_wrist_roll', 'left_wrist_roll'])

        self.populate_latest("wrist_r", r["right_wrist"][0])
        ...
        for i, name in enumerate(fingers):
            if name != "none":
                self.populate_latest(name + "_r", r["right_fingers"][i])

            
        self.est_hand_pos = get_hand_joints(self.latest)



        #################################################################

        self.wrist_pose = list(self.latest["wrist_r"][0]) + list(self.latest["wrist_r"][1])
